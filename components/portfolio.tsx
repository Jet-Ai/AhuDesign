  ...
  return <main id="top">
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <header className="header">
        ...
        <nav aria-label={t("Main navigation")}><a href="#projects">{t("Projects")}</a><a href="#studio">{t("Studio")}</a><a href="#contact">{t("Contact")}{' '}<ArrowUpRight size={15}/></a></nav><nav className="language-switch" aria-label={t('Language')}>{(['sk','az','en'] as const).map(code=><a key={code} href={'/'+code} hrefLang={code} lang={code} aria-current={locale===code?'page':undefined} aria-label={{sk:'Slovenčina',az:'Azərbaycanca',en:'English'}[code]}>{code.toUpperCase()}</a>)}</nav>
      </header>
    ...
    <section id="studio" className="studio section-pad">
      <p className="eyebrow">{t("01 / THE STUDIO")}</p>
    ...
    <footer id="contact" className="footer section-pad"><p className="eyebrow">{t("04 / YOUR VISION. OUR EXPERIENCE.")}</p><h2>{t("Let's make")}<br/>{t("room for")}{' '}<em>{t("your ideas.")}</em></h2><div className="contact-line"><a href="mailto:hasan.alibeyli@outlook.com">hasan.alibeyli@outlook.com <ArrowUpRight size={20}/></a><a href="tel:+421944693002">+421 944 693 002 <ArrowUpRight size={20}/></a></div><div className="footer-bottom"><a href="#top" className="wordmark">AHU<span>DESIGN</span></a><p>© {new Date().getFullYear()} AHU Design</p><a className="back-top" href="#top">{t("Back to top")}{' '}<ArrowUpRight size={17}/></a></div></footer>

    <Dialog open={selected!==null} onOpenChange={open=>{if(!open)setSelected(null)}}><DialogContent className="project-dialog" showCloseButton={false}><DialogClose className="localized-close" aria-label={t('Close')}>×</DialogClose>{project&&<><div className="dialog-info"><p className="eyebrow">{project.type}</p><DialogTitle className="dialog-title">{project.title}</DialogTitle><DialogDescription className="dialog-description">{project.text}</DialogDescription></div><img className="dialog-image" src={project.images[slide]} alt={project.title+t(' view ')+(slide+1)}/>{project.images.length>1&&<div className="gallery-controls"><button aria-label={t("Previous image")} onClick={()=>setSlide((slide+project.images.length-1)%project.images.length)}><ArrowLeft/></button><span>{slide+1} / {project.images.length}</span><button aria-label={t("Next image")} onClick={()=>setSlide((slide+1)%project.images.length)}><ArrowRight/></button></div>}</>}</DialogContent></Dialog>
  </main>;
}
